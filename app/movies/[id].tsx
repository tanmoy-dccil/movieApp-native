...
const MovieInfo = ({ label, value }: MovieInfoProps) => (
  <View className='flex-col items-start justify-center mt-5'>
    ...
    <Text className='text-lime-50 font-bold text-sm mt-2'>
      {value || 'N/A'}
    </Text>
    ...
    <View className='bg-primary flex-1'>
      <ScrollView contentContainerStyle={{
        ...
        <View className='flex-col item-start justify-center mt-5 px-5'>
          ...
          <View className='flex felx-row justify-between w-1/2'>
            <MovieInfo label='Budget' value={`$${movie?.budget / 1000000}million`} />
            <MovieInfo label='Revenue' value={`$${Math.round(movie?.revenue) / 1000000}`} />
          </View>
          <MovieInfo label='Product Companies' value={movie?.production_companies.map((c) => c.name).join(' - ') || 'N/A'} />
        </View>
      </ScrollView>
      <TouchableOpacity className='absolute bottom-5 left-0 right-0 mx-5
      bg-accent rounded-lg py-3.5 flex flex-row items-center justify-center z-50'
        onPress={router.back}>
